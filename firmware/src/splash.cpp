...
void splash_init(lv_obj_t *parent) {
    ...
    lv_obj_set_style_text_font(label_status, theme_font_display_s(), 0);
...
void splash_tick(void) {
    // P2-2: hard early-return when splash isn't the active screen. The
    // `active` flag below is set/cleared by splash_show/_hide and tracks the
    // same condition, but this is the authoritative source of truth and
    // makes the gating obvious — the tick is essentially free when the user
    // is on USAGE / SESSION / PET / MENU.
...
void splash_pick_for_current_rate(void) {
    ...
    // Pinned override from menu Splash picker — short-circuit the
    // rate-driven selection if the user has locked one animation in.
    int8_t pin = settings::splash_pin();
    if (pin >= 0 && pin < (int)SPLASH_ANIM_COUNT) {
        cur_anim = (uint16_t)pin;
        cur_frame = 0;
        frame_started_ms = millis();
        last_pick_ms = frame_started_ms;
        const splash_anim_def_t *a = &splash_anims[cur_anim];
...
bool splash_render_to_buf(uint16_t* dest, int dest_w,
                          ...
                          bool shiny, bool dim)
{
    if (!dest || cell_size <= 0 || dest_w < GRID * cell_size) return false;
    if (anim_idx >= SPLASH_ANIM_COUNT) return false;
    const splash_anim_def_t& a = splash_anims[anim_idx];
    if (a.frame_count == 0) return false;
    const uint8_t* cells = a.frames[frame_idx % a.frame_count];
    const uint16_t* base_palette = a.palette;

    // Build a local palette (10 entries) so we only apply transforms once
    // per render rather than once per pixel. Equivalent output, much cheaper.
    // shiny: 0x8410 flips the top bit of each channel in RGB565:
    //   bit 15 = top R bit, bit 10 = top G bit, bit 4 = top B bit.
    // dim: halve R, G, B components (sad/neglected appearance). Applied after
    //   the shiny XOR so both transforms compose correctly.
    ...
    for (int gy = 0; gy < GRID; gy++) {
        for (int gx = 0; gx < GRID; gx++) {
            uint8_t code = cells[gy * GRID + gx];
            uint16_t color = (code < SPLASH_PALETTE_SIZE) ? local_palette[code] : COL_EMPTY;
            for (int py = 0; py < cell_size; py++) {
                uint16_t* row = dest + (gy * cell_size + py) * dest_w + gx * cell_size;
                for (int px = 0; px < cell_size; px++) row[px] = color;
            }
        }
    }
    return true;
...
int splash_pick_in_group(uint8_t group, uint8_t rotation_offset) {
    ...
    uint8_t n = group_size[group];
    if (n == 0) return -1;
    return group_lists[group][rotation_offset % n];
}
