...
static int      g_sim_batt_override = -1;   // -1 = off; 0..100 = forced pct
...
int  power_hal_battery_pct(void) {
    return (g_sim_batt_override >= 0) ? g_sim_batt_override : cached_pct;
}
...
void power_hal_set_sim_battery(int pct) {
    if (pct < 0) {
        g_sim_batt_override = -1;
    } else {
        if (pct > 100) pct = 100;
        g_sim_batt_override = pct;
    }
}
